...
insertShelleyBlock tracer lenv firstBlockOfEpoch blk lStateSnap details = do
  runExceptT $ do
    ...
    let insertTx' = insertTx tracer (leNetwork lenv) lStateSnap blkId (sdEpochNo details) (Generic.blkSlotNo blk)
    grouped <- foldM (\grouped (idx, tx) -> insertTx' idx tx grouped) mempty zippedTx
    ...
    case qres of
      ...
      Left err ->
        case List.find matches groupedOutputs of
          ...
          Just eutxo -> pure $ Right $
            (DB.txOutTxId (etoTxOut eutxo), DB.txOutValue (etoTxOut eutxo))
  where
